...
function FormComponent(props: FormProps) {
  ...
  function onSubmit(event: any) {
    ...
    if (props.sendSubmissionsTo === 'zapier') {
      ...
      /* TODO: throw error if no action URL */ logFetch(url);
      fetch(url, { body, headers, method: props.method || 'post' }).then(
        async res => {
          let body;
    ...
    <>
      ...
      >
        ...
        {submissionState() === 'error' ? (
          <Blocks path="errorMessage" blocks={props.errorMessage!} context={props.builderContext} />
        ) : null}
        ...
        {submissionState() === 'error' && responseData ? (
          <pre className="builder-form-error-text pre-7de29bd8">
      ...
      <style>{`.pre-7de29bd8 {   padding: 10px;   color: red;   text-align: center; }`}</style>{' '}
    </>
  );
}
